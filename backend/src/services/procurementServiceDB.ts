...
import { getProcurementSparkline } from './procurementSnapshotService'
import type {
  ProcurementHomepageData, ProcurementKpis, ApprovalQueueItem,
  OverduePO, CriticalShortage, VendorBar, VendorMode,
  SpendGauge, SpendCategory, ActionQueue, ExpectedReceipt,
  AlertBanner, PODetail, ProcurementActionResult, Rag,
  SpendKpiTile, SpendModeStat,
...
function ragFromOverdue(days: number): Rag {
  return days > 7 ? 'red' : days >= 3 ? 'amber' : 'green'
}

function ragFromPct(pct: number): Rag {
  return pct > 95 ? 'red' : pct >= 75 ? 'amber' : 'green'
}

// Indian financial year quarter label from a date
function fyQuarterLabel(date: Date): string {
...
async function getKpis(): Promise<ProcurementKpis> {
  ...
  const [prsPending, openPOs, overduePOs, criticalStock, spendMtd, spendTrend, budgetAll, poRaisedTrend] =
    await Promise.all([
      ...
      // 6-month trend for spend sparkline
      query<{ month: string; spend: number }>(
      ...
      // Total budget for current month (all categories)
      query<{ budget: number }>(
      ...
      // Live proxy trend: POs raised per month (used for count-tile sparklines when snapshots are sparse)
      query<{ month: string; cnt: number }>(
  ...
  // Build exactly 6 monthly slots ending this month, filling 0 for months with no data
  function buildSixMonthSlots<T extends { month: string }>(
    rows: T[],
    getValue: (r: T) => number,
  ...
  // Live proxy trend for count tiles (POs raised per month) — used until 6+ monthly snapshots exist
  const poProxyTrend = buildSixMonthSlots(poRaisedTrend, r => Number(r.cnt))

  function countSpark(field: 'prsPending' | 'openPOs' | 'overduePOs' | 'criticalStock') {
    const fromSnapshot = getProcurementSparkline(field)
  ...
  // Build M/Q/Y spend mode stats — Q and Y use live data where possible
  const byMode = await buildSpendByMode(spentMtd, budgetMtd, spendPctMtd, month, year, spendSparkFromDB)
...
async function buildSpendByMode(
  ...
  // Q: spend for each FY quarter in current FY
  const [qSpend, qBudget, ySpend, yBudget] = await Promise.all([
    query<{ qtr: number; spend: number }>(
    ...
    // Y: last 3 financial years total spend
    query<{ fy: number; spend: number }>(
      `SELECT
    ...
    query<{ year: number; budget: number }>(
  ...
  const qVals   = [0,0,0,0]
  // April=Q1, July=Q2, Oct=Q3, Jan=Q4 (Indian FY)
  const fyQtrMap: Record<number, number> = { 1: 3, 2: 0, 3: 1, 4: 2 } // QUARTER() → FY Q index
  ...
  const yPct        = latestBudget > 0 && latestFY
    ? Math.round((latestFY.spend / latestBudget) * 100) : 0
...
async function getApprovalQueue(): Promise<ApprovalQueueItem[]> {
  const rows = await query<{
    po_no: string; requester: string; department: string
    first_item: string; supplier: string; required_by: string
    est_value: number; workflow_state: string; days_pending: number
  }>(
    `SELECT
       po.name                                          AS po_no,
...
async function getOverduePOs(): Promise<OverduePO[]> {
  const rows = await query<{
    po_no: string; supplier: string; po_value: number
    schedule_date: string; days_overdue: number
    per_received: number; last_followup: string | null
  }>(
    `SELECT
       po.name                                          AS po_no,
       po.supplier,
  ...
  return rows.map(r => ({
    ...
    rag:          ragFromOverdue(Number(r.days_overdue)),
  }))
}

// ── W-PROC-08 Critical material shortage ─────────────────────────────────────

async function getCriticalShortages(): Promise<CriticalShortage[]> {
  const rows = await query<{
    wo_no: string; blocked_item: string; required_qty: number
    available_qty: number; shortfall: number
    planned_end_date: string | null; eta_from_po: string | null
  }>(
    `SELECT
       wo.name                                          AS wo_no,
...
function vendorDateFilter(mode: VendorMode): [string, string] {
  const now     = new Date()
  const fyStart = now.getMonth() >= 3 ? now.getFullYear() : now.getFullYear() - 1
  if (mode === 'Q') {
...
async function getVendorPerformance(): Promise<Record<VendorMode, VendorBar[]>> {
  ...
  const [M, Q, Y] = await Promise.all([
    fetchMode('M'), fetchMode('Q'), fetchMode('Y'),
  ])
  return { M, Q, Y }
}
...
async function getSpendGauge(): Promise<SpendGauge> {
  const now   = new Date()
  const month = now.toLocaleString('default', { month: 'long' })
  const year  = now.getFullYear()
...
async function getActionQueue(): Promise<ActionQueue> {
  const [grns, followUps, invoices] = await Promise.all([
    ...
    // Tab 1: GRNs pending (due today or earlier)
    query<{ po_no: string; supplier: string; first_item: string; schedule_date: string; per_received: number }>(
    ...
    // Tab 2: PO follow-ups due (no comment in last 3 days)
    query<{ po_no: string; supplier: string; schedule_date: string; last_followup: string | null; days_overdue: number }>(
      `SELECT
...
// ── W-PROC-12 Expected receipts this week ────────────────────────────────────

async function getExpectedReceipts(): Promise<ExpectedReceipt[]> {
  const rows = await query<{
    supplier: string; po_no: string; schedule_date: string
    per_received: number; last_followup: string | null
  }>(
    `SELECT
  ...
  return rows.map(r => {
    const daysUntil = Math.ceil(
      (new Date(r.schedule_date).getTime() - today.getTime()) / 86_400_000,
    )
    const hasRecentFollowup = r.last_followup
      ? (today.getTime() - new Date(r.last_followup).getTime()) < 2 * 86_400_000
      : false
...
export async function getPODetail(poName: string): Promise<PODetail | null> {
  const [poRows, itemRows] = await Promise.all([
    query<{
      name: string; supplier: string; owner: string; department: string
      workflow_state: string; grand_total: number
      schedule_date: string; per_received: number; last_followup: string | null
    }>(
      `SELECT
         po.name, po.supplier,
    ...
    query<{ item_code: string; item_name: string; qty: number; rate: number; amount: number; uom: string }>(
...
export async function approvePO(poName: string): Promise<ProcurementActionResult> {
  return frappePost<ProcurementActionResult>(
    'proman_edge.api.procurement.approve_purchase_order',
    { name: poName },
  )
}

export async function returnPO(poName: string, reason: string): Promise<ProcurementActionResult> {
  return frappePost<ProcurementActionResult>(
    'proman_edge.api.procurement.return_purchase_order',
    { name: poName, reason },
  )
}
...
export async function makeGRN(poName: string): Promise<ProcurementActionResult> {
  return frappePost<ProcurementActionResult>(
    'proman_edge.api.procurement.make_grn',
    { name: poName },
  )
}
...
export async function getProcurementHomepage(): Promise<ProcurementHomepageData> {
  ...
  const alerts = buildAlerts(criticalShortages, approvalQueue, base)
