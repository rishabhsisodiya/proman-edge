// Reads Finance Head data for sites the client cannot give us DB access to (Tally-based).
// Client fills the workbook in backend/data/site-sheets/<Site>.xlsx and we parse it here.
//
// Kept fully separate from financeServiceDB.ts (the live-DB path for PISPL) on purpose —
// this ingestion mechanism (local xlsx today) is expected to change (e.g. pull from Google
// Sheets API instead) without touching the ERPNext/MariaDB query logic.
import path from 'path'
import XLSX from 'xlsx'
import type {
  EntityAmount, EntityAmountWithTrend, CashBankAccount, AgeingBucket, TopDebtor,
  GrossMarginEntity, PayablesInvoiceRow, UnpaidInvoice, JournalEntryPending, PoApprovalItem,
  FinanceHomepageData, Period, PeriodStat,
} from '../types/finance'
import { getGmTargetPct } from './financeSettingsService'

// Local copy of financeServiceDB's periodRange (calendar M / fiscal Q-Y, India Apr–Mar) —
// duplicated rather than imported to avoid a circular dependency between the two services.
...
const SITE_SHEET_DIR = path.join(__dirname, '../../data/site-sheets')

// Entity label (matches the frontend's ENTITY_LABELS) -> workbook filename
...
function readTab(company: string, tab: string): Record<string, unknown>[] {
  ...
  try {
    ...
    return [] // sheet missing/unreadable — treat as no data for this site, not a crash
  }
}
...
export function getSiteSheetCashBank(): { byEntity: EntityAmountWithTrend[]; accountsByEntity: Record<string, CashBankAccount[]>; total: number } {
  ...
  for (const entity of getSiteSheetCompanies()) {
    ...
    byEntity.push({ entity, value, changeVs7d: 0 }) // no historical snapshot from a static sheet yet
    total += value
  }
...
// ── Overdue receivables (net, via outstanding_amount + due_date — no PLE concept in Tally) ──
...
export function getSiteSheetOverdueReceivables(): { total: number; over90: number; over90Count: number; byEntity: (EntityAmount & { over90: number })[] } {
  ...
  let total = 0, over90 = 0, over90Count = 0
  const byEntity = getSiteSheetCompanies().map(entity => {
    ...
    let entityTotal = 0, entityOver90 = 0
    for (const r of rows) {
      ...
      if (daysOverdue > 90) { entityOver90 += outstanding; over90Count++ }
    }
    total += entityTotal
    over90 += entityOver90
  ...
  for (const entity of getSiteSheetCompanies()) {
    readTab(entity, 'Purchase Invoices').forEach(r => {
      ...
      if (outstanding > 0 && Math.round(outstanding) >= Math.round(grandTotal)) {
        ...
        paymentsToRelease.push({
          invoiceNo: str(r.invoice_no), vendor: str(r.supplier), amount: outstanding,
          dueDate: str(r.due_date), daysOverdue: Math.floor((today.getTime() - dueDate.getTime()) / 86_400_000), entity,
        })
      }
    })
    readTab(entity, 'Journal Entries').forEach(r => {
      ...
      journalEntriesPending.push({
        name: str(r.je_no), userRemark: '', totalDebit: num(r.total_debit), voucherType: str(r.voucher_type),
        daysPending: Math.floor((today.getTime() - posting.getTime()) / 86_400_000), entity,
      })
    })
  }
  ...
  for (const entity of getSiteSheetCompanies()) {
    readTab(entity, 'Purchase Orders')
      ...
      .forEach(r => {
        ...
        items.push({
          poNo: str(r.po_no), vendor: str(r.supplier), value: num(r.grand_total),
          approvalStage: str(r.workflow_state), poDate: str(r.po_date),
          daysPending: Math.floor((today.getTime() - poDate.getTime()) / 86_400_000), entity,
        })
      })
  }
  return items
...
export function getSiteSheetGrossMarginByEntity(start: string, end: string): GrossMarginEntity[] {
  return getSiteSheetCompanies().map(entity => {
    ...
    return { entity, income, expense, gmPct: gmPct(income, expense), targetPct: getGmTargetPct(entity) }
  })
}
  ...
  for (const { type, label, dueDay } of STATUTORY_TYPES) {
    ...
    for (const entity of getSiteSheetCompanies()) {
      readTab(entity, 'GL Summary')
        ...
        .forEach(r => { amount += num(r.credit) - num(r.debit) })
    }
...
// ── Merge into the DB-based homepage payload ─────────────────────────────────
// One entry point, called once from financeServiceDB.getFinanceHomepage(). Swapping the
// ingestion mechanism later (e.g. Google Sheets API pull) only means rewriting this file —
// financeServiceDB.ts and its ERPNext/MariaDB queries stay untouched.
...
export function mergeSiteSheetsIntoHomepage(base: FinanceHomepageData): FinanceHomepageData {
  ...
  function mergeGrossMarginStat(stat: FinanceHomepageData['grossMargin']['M'], period: Period) {
    const { start, end } = periodRange(period)
    const sheetByEntity = getSiteSheetGrossMarginByEntity(start, end)
    ...
    return {
      ...
      byEntity, periodLabel: stat.periodLabel,
    }
  }
  const grossMargin = {
  ...
  return {
    ...
    cashBank, revenue, overdueReceivables, receivablesAgeing, gstLiability, payablesDue7d, payablesInvoices14d,
    actionQueue, poApprovalQueue, grossMargin, alerts,
  }
}
